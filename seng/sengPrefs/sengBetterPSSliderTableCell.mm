...
#import "sengBetterPSSliderTableCell.h"
#import "Controllers.h"

@implementation sengBetterPSSliderTableCell
...
- (void)presentPopup {
    alert = [[UIAlertView alloc] initWithTitle:localisedStringForKey(self.specifier.name)
             message:[NSString localizedStringWithFormat:localisedStringForKey(@"al_ENTER_VALUE"), [[self.specifier propertyForKey:@"min"] floatValue], [[self.specifier propertyForKey:@"max"] floatValue]]
             delegate:self
             cancelButtonTitle:localisedStringForKey(@"al_CANCEL")
             otherButtonTitles:localisedStringForKey(@"al_ENTER")
             , nil];
    ...
    if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad && (needsNegate || needsPoint)) {
        ...
        UIBarButtonItem *buttonOne = [[UIBarButtonItem alloc] initWithTitle:localisedStringForKey(@"al_NEGATE") style:UIBarButtonItemStylePlain target:self action:@selector(typeMinus)];
        UIBarButtonItem *buttonTwo = [[UIBarButtonItem alloc] initWithTitle:localisedStringForKey(@"al_POINT") style:UIBarButtonItemStylePlain target:self action:@selector(typePoint)];
        NSArray *buttons = nil;
    ...
    if (alertView.tag == 342879) {
        if (buttonIndex == 1) {
            ...
            if (value <= [[self.specifier propertyForKey:@"max"] floatValue] && value >= [[self.specifier propertyForKey:@"min"] floatValue]) {
                ...
                UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:localisedStringForKey(@"al_ERROR")
                                           message:localisedStringForKey(@"al_VALID_VALE")
                                           delegate:self
